                    ...
                    currentShipment.customTasks.map((task) => (
                      <div key={task.id} className="flex items-center justify-between group p-2 rounded-md hover:bg-muted/50 transition-colors">
                        <div className="flex items-center space-x-3">
                          ...
                          <Label
                            htmlFor={task.id}
                            className={`text-sm font-medium leading-none cursor-pointer ${task.completed ? 'text-muted-foreground line-through decoration-muted-foreground/50' : ''
                              }`}
                          >
                            {task.text}
                          </Label>
                    ...
                    <div key={item.id} className="flex items-center justify-between group p-2 rounded-md hover:bg-muted/50 transition-colors">
                      <div className="flex items-center space-x-3">
                        ...
                        <Label
                          ...
                          className={`text-sm leading-none cursor-pointer ${item.completed ? 'text-muted-foreground line-through decoration-muted-foreground/50' : ''
                            }`}
                        >
                          {item.item}
                        </Label>
                  ...
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && newChecklistInput.trim()) {
                        addShipmentChecklistItem(currentShipment.id, newChecklistInput.trim());
